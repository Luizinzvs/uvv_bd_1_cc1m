...
create role luizfelipe with superuser createdb createrole inherit login replication bypassrls encrypted password '202202858';
/* Criando a minha database */
create database uvv
with
owner = luizfelipe
template = template0
...
CREATE SCHEMA hr AUTHORIZATION luizfelipe;
/* Setando o esquema hr como padrão */
SELECT CURRENT_SCHEMA();
SET SEARCH_PATH TO hr, "$user", public;
SHOW SEARCH_PATH;
ALTER USER luizfelipe
SET SEARCH_PATH TO hr, "$user", public;
...
COMMENT ON TABLE cargos IS 'Tabela cargos, que armazena os cargos da empresa e a média salarial (mínimo e máximo) para cada cargo existente na empresa.';
COMMENT ON COLUMN cargos.id_cargo IS 'É a chave primária da tabela cargos.';
COMMENT ON COLUMN cargos.cargo IS 'É uma chave unica da tabela cargos. Nome dos cargo existentes da empresa.';
COMMENT ON COLUMN cargos.salario_minimo IS 'O menor salário de um cargo.';
COMMENT ON COLUMN cargos.salario_maximo IS 'O maior salário de um cargo.';
...
COMMENT ON TABLE regioes IS 'Tabela regiões em que contém o nome das regiões.';
COMMENT ON COLUMN regioes.id_regiao IS 'É a chave primária da tabela regiões.';
COMMENT ON COLUMN regioes.nome IS 'chave única da tabela regiões.';
...
COMMENT ON TABLE paises IS 'Tabela países em que contém as informações dos países.';
COMMENT ON COLUMN paises.id_pais IS 'É a chave primária da tabela países.';
COMMENT ON COLUMN paises.nome IS 'É a chave unica da tabela paises.';
COMMENT ON COLUMN paises.id_regiao IS 'É uma chave estrangeira para a tabela de regiões.';
...
COMMENT ON TABLE localizacoes IS 'Tabela localizações em que contém varios endereços de escritórios e dados da empresa. Não armazena os endereços dos clientes.';
COMMENT ON COLUMN localizacoes.id_localizacao IS 'É uma chave primária da tabela localizações.';
COMMENT ON COLUMN localizacoes.endereco IS 'Endereço de um escritório ou facilidade da empresa.';
COMMENT ON COLUMN localizacoes.cep IS 'É o CEP da localização de algum certo escritório da empresa.';
COMMENT ON COLUMN localizacoes.cidade IS 'Cidade onde está localizado o escritório da empresa.';
COMMENT ON COLUMN localizacoes.uf IS 'Estado onde está localizado o escritório da empresa.';
COMMENT ON COLUMN localizacoes.id_pais IS 'É uma chave estrangeira para a tabela países.';
...
/* Comentários referentes a tabela de localizações e suas colunas */
COMMENT ON TABLE departamentos IS 'É a tabela que contém as informações sobre os departamentos da empresa.';
COMMENT ON COLUMN departamentos.id_departamento IS 'É a chave primária da tabela departamentos.';
COMMENT ON COLUMN departamentos.nome IS 'la departamentos.';
COMMENT ON COLUMN departamentos.id_localizacao IS 'É a chave estrangeira para a tabela de localizações.';
COMMENT ON COLUMN departamentos.id_gerente IS 'É a chave estrangeira para a tabela de empregados.';
...
COMMENT ON TABLE empregados IS 'É a tabela que contém todas as informações dos empregados.';
COMMENT ON COLUMN empregados.id_empregado IS 'É a chave primária da tabela empregados.';
COMMENT ON COLUMN empregados.nome IS 'Nome completo de um empregado.';
COMMENT ON COLUMN empregados.email IS 'Parte inicial (Anterior ao @) do email de um empregado.';
COMMENT ON COLUMN empregados.telefone IS 'Telefone do empregado com o DDD e o código do País.';
COMMENT ON COLUMN empregados.data_contratacao IS 'Data em que o empregado foi contratado.';
COMMENT ON COLUMN empregados.id_cargo IS 'Cargo do empregado.';
COMMENT ON COLUMN empregados.salario IS 'Indica o salário mensal atual de um empregado.';
COMMENT ON COLUMN empregados.comissao IS 'Comissão de um empregado. Disponivel apenas para empregados do departamento de vendas.';
COMMENT ON COLUMN empregados.id_departamento IS 'É uma chave estrangeira para a tabela de departamentos. É o departamento atual do funcionario.';
COMMENT ON COLUMN empregados.id_supervisor IS 'Indica um auto-relacionamento com a própria tabela empregados. ';
...
COMMENT ON TABLE historico_cargos IS 'Tabela que armazena o histórico de cargos de um empregado. Caso o empregado mude seu cargo ou seu departamento, novas linhas deverão ser inseridas nessa tabela com as informações antigas dele.';
COMMENT ON COLUMN historico_cargos.id_empregado IS 'É parte de uma chave primária da tabela historico_cargos. Também é uma chave estrangeira para a tabela empregados.';
COMMENT ON COLUMN historico_cargos.data_inicial IS 'Chave primária da tabela historico cargos. Data em que o empregado iniciou no novo cargo. Deve ser menor do que a data final.';
COMMENT ON COLUMN historico_cargos.data_final IS 'Ultimo dia em que o empregado permaneceu no cargo. Deve ser maior que a data inicial.';
COMMENT ON COLUMN historico_cargos.id_cargo IS 'É uma chave estrangeira para a tabela de departamentos. Departamento em que o empregado trabalhou no passado.';
COMMENT ON COLUMN historico_cargos.id_departamento IS 'É uma chave estrangeira para a tabela de departamentos. Departamento em que o empregado trabalhou no passado.';
